...
class DeepModel(torch.nn.Module):
    ...
    def __init__(self, user_num, item_num, u_vector_size, i_vector_size, model_path, layers, smooth_coef=0.1,
                 loss_func='BCE'):
    ...
    def forward(self, feed_dict):
        ...
        if 'mse' in self.loss_func:
            ...
            loss = torch.mean(
                -y * torch.log(fpred) * torch.pow((1 - fpred), 2) - (1 - y) * torch.log(1 - fpred) * torch.pow(fpred,
                                                                                                               2))
            ...
            loss = torch.mean(
                torch.log(torch.ones_like(pred) / (torch.min(y, pred) + self.smooth_coef) + self.smooth_coef) * (
                            torch.min(y, pred) + y + pred))
        else:
