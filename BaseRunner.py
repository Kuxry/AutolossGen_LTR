...
from ltr import NeuralRanker, load_multiple_data, metric_results_to_string, evaluation, \
    DataProcessor, RankMSE, DataProcessor_epoch
...
class baserunner():
    ...
    def fit(self,model,data_id=None, dir_data=None,loss_fun=None,sample_arc=None, regularizer=True):
        epoch = 1
        num_queries = 0
        fold_num=5
        model.train_mode()
    ...
    def fold_train(self, search_loss=False,model_id=None,data_id=None, dir_data=None,skip_eval=0,fold=None):
        logging.info(fold)
        ...
        # 获得训练、验证、测试数据，epoch=-1不shuffle
        train_data, test_data,validation_data = DataProcessor_epoch(data_id, dir_data, fold)
        ...
        try:
            for epoch in range(self.epoch):
                ...
                self.controller.zero_grad()
                if search_loss:
                    start_auc=pre_evaluate(model,validation_data)#
                    baseline = torch.tensor(start_auc).cuda()
                    cur_model= copy.deepcopy(model)
                    grad_dict = dict()
                    test_pred = torch.rand(20).cuda() * 0.8 + 0.1  # change range here
                    ...
                    for i in tqdm(range(last_search_cnt), leave=False, desc='Epoch %5d' % (epoch + 1), ncols=100,mininterval=1):
                        while True:
                            ...
                            if reward is None:
                                ...
                                reward = torch.tensor(pre_evaluate(model,validation_data,True,True)).cuda()
                                grad_dict[test_pred.grad.clone().detach()] = reward.clone().detach()
                                model = copy.deepcopy(cur_model)
                            if reward < baseline - self.args.skip_lim:
                                reward = min_reward.clone().detach()
                    ...
                    if self.args.train_with_optim and best_arc is not None and max_reward > start_auc - self.args.rej_lim:
                        ...
                        for j in range(self.args.search_train_epoch):
                            last_batch = self.fit(model,train_data,loss_fun=self.loss_formula, sample_arc=sample_arc, regularizer=False)
                        new_auc = torch.tensor(pre_evaluate(model,validation_data,True,True)).cuda()
                        print('Optimal: ', self.loss_formula.log_formula(sample_arc=sample_arc, id=self.loss_formula.num_layers - 1))
                    else:
                        grad_dict = dict()
                        self.controller.zero_grad()
...
def pre_evaluate(model,data,reward_check=False,result_check=False):
